...
class _SessionTablePageState extends State<SessionTablePage> {
  ...
  Future<void> _loadSessions() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        Utils.showSnackBar('Erreur lors du chargement des données', context);
      }
    }
  }
  ...
  Future<void> _syncSession(Session session) async {
    ...
    try {
      ...
      if (success) {
        ...
        if (mounted) {
          Utils.showSnackBar('Données synchronisées', context);
          _loadSessions();
        }
      } else {
        if (mounted) {
  ...
  Future<void> _syncAllNotSynced() async {
    ...
    if (notSyncedSessions.isEmpty) {
      Utils.showSnackBar('Toutes les données sont déjà synchronisées', context);
      return;
    }
